<template>
	<view class="content">
		...
		<view v-for="(item,index) in audioList" :key="index">
			...
			<view class="chat-voice-right" :style="{width:handleVoiceWidth(item.message.length)}"
				@tap="handleVoicePlay(item)">
				<view class="chat-voice-right-inner">
					<!-- 时长 -->
					<text decode='true' class="chat-voice-length-right">{{item.message.length}}</text>
					<!-- 语音条主体 -->
		...
		<view class="bottomContent" v-if="activeName === 'videoChat'">
			<img src="/static/imgs/fontChat.png" class="close-icon" @click="toFontChat" />
			<button size="mini" class="f3" @touchend="handleTouchEnd" @touchmove="handleTouchMove"
				@touchstart="handleTouchStart">
				按住说话
			</button>
		</view>
		<view class="bottomContent" v-if="activeName === 'fontChat'">
			<img src="/static/imgs/videoChat.png" class="close-icon" @click="toVideoChat" />
			<input
				class="uni-input f3"
